...
export interface GraphChoice { text: string; nextSceneId: string; }
...
const exportAsScenes = (nodes: FlowNode<EditableNodeData>[], edges: FlowEdge[]): SceneRequest[] =>
    nodes.map((node) => {
...
const toRequests = exportAsScenes;

const AdminGraph: React.FC<AdminGraphProps> = ({ useStory, useSaveBulk, useDeleteScene, selectScenes, backPath }) => {
    ...
    const { data } = useStory();
    const { mutate: saveBulk, isPending } = useSaveBulk();
    const { mutate: deleteScene, isPending: isDeletePending } = useDeleteScene();
    ...
    useEffect(() => {
        if (!data) return;
        const scenes = selectScenes(data);
        const loadedNodes = scenes.map((scene, idx) => ({
            id: scene.sceneId,
            type: 'editableNode',
        ...
        const loadedEdges = scenes.flatMap((scene) =>
            scene.choices.map((choice, idx) => ({
                id: `${scene.sceneId}-${choice.nextSceneId}-${idx}`,
    ...
    const onConnect = useCallback((connection: Connection) => {
        ...
        setEdges((eds) => assignOffsets([...eds, newEdge]));
    }, [setEdges]);

    const onSelectionChange = useCallback((sel: { nodes: FlowNode[]; edges: FlowEdge[] }) => {
        setSelection(sel);
